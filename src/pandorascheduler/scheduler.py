...
sched_stop    = '2025-12-20 00:00:00'


def Schedule(pandora_start:str, pandora_stop:str, obs_window:timedelta, 
            transit_coverage_min:float, sched_start=None, sched_stop=None):
    ...
    # Import TOO/hard schedule events
    if os.path.exists(f'{PACKAGEDIR}/data/TOO_list.csv') == True:
        TOOs = pd.read_csv(f'{PACKAGEDIR}/data/TOO_list.csv', sep=',')
        TOO_targets = TOOs['Target']
        TOO_starts  = [datetime.strptime(d, '%Y-%m-%d %H:%M:%S') for d in TOOs['Observation Start']]
        TOO_stops   = [datetime.strptime(d, '%Y-%m-%d %H:%M:%S') for d in TOOs['Observation Stop']]
...
### Check if previous observations already exist and if so update tracker
    if os.path.exists(f'{PACKAGEDIR}/data/Pandora_Schedule.csv') == True:
        archive = pd.read_csv(f'{PACKAGEDIR}/data/Pandora_Schedule.csv',  sep=',')
        ### ADD: Warning if conflict between scheduling start and previous observations
        if len(archive) > 0:
            for i in range(len(archive)):
                tracker.loc[(tracker['Planet Name'] == archive['Target'][i]), 'Transits Needed'] = \
                    (tracker.loc[(tracker['Planet Name'] == archive['Target'][i])]['Transits Needed']-1)
                tracker.loc[(tracker['Planet Name'] == archive['Target'][i]), 'Transits Acquired'] = \
                    (tracker.loc[(tracker['Planet Name'] == archive['Target'][i])]['Transits Acquired']+1)
...
### and the specified observation window if different
    ...
    for i in range(len(target_data)):
        planet_name = target_data['Planet Name'][i]
        star_name   = target_data['Star Name'][i]
        planet_data = pd.read_csv(f'{PACKAGEDIR}/data/targets/{star_name}/{planet_name}/Visibility for {planet_name}.csv')
        planet_data = planet_data.drop(planet_data.index[(planet_data['Transit_Coverage'] < 
                                        transit_coverage_min)]).reset_index(drop=True)
        ...
        p_trans = planet_data.index[(pandora_start <= start_transits) & (end_transits <= pandora_stop)]
        s_trans = planet_data.index[(sched_start <= start_transits) & (end_transits <= sched_stop)]
        pandora_transits_left.append(len(p_trans))
        schedule_transits_left.append(len(s_trans))
        
    pandora_transits_left  = pd.DataFrame(pandora_transits_left,  columns=['Transits Left in Lifetime'])
    schedule_transits_left = pd.DataFrame(schedule_transits_left, columns=['Transits Left in Schedule'])
    tracker = pd.concat([tracker, pandora_transits_left, schedule_transits_left], axis=1)

### Calculate an initial priority number for each planet
    trans_priority = []
    for i in range(len(tracker)):
        trans_priority.append(tracker['Transits Left in Lifetime'][i] - tracker['Transits Needed'][i])
...
### Begin scheduling
    ...
    while (stop <= sched_stop):
        ...
        temp_df = pd.DataFrame([], columns=['Planet Name', 'Obs Start', 'Obs Gap Time', 
                                                    'Schedule Factor', 'Transit Factor', 'Quality Factor'])

### First check if a TOO is within observing window
        overlap_TOO = obs_rng.intersection(TOO_starts)
        if len(overlap_TOO) > 0:
            obs_start   = TOO_starts[TOO_starts.index(overlap_TOO[0])]
            obs_stop    = TOO_stops[TOO_starts.index(overlap_TOO[0])]
            planet_name = TOO_targets[TOO_starts.index(overlap_TOO[0])]
...
### Next look at each planet and determine if transit occurs in window
        for i in range(len(tracker)):
            ...
            if (tracker.loc[(tracker['Planet Name'] == planet_name), 'Transits Needed'][i] == 0):
                pass

            else:
                star_name = target_data['Star Name'][np.where(target_data['Planet Name']==planet_name)[0][0]]
                planet_data = pd.read_csv(f'{PACKAGEDIR}/data/targets/{star_name}/{planet_name}/Visibility for {planet_name}.csv')
                planet_data = planet_data.drop(planet_data.index[(planet_data['Transit_Coverage'] < 
                                                transit_coverage_min)]).reset_index(drop=True)
                start_transits = Time(planet_data['Transit_Start'], format='mjd', scale= 'utc').to_value('datetime')
                ...
                p_trans = planet_data.index[(pandora_start <= start_transits) & (end_transits <= pandora_stop)]
                s_trans = planet_data.index[(sched_start <= start_transits) & (end_transits <= sched_stop)]
                tracker.loc[(tracker['Planet Name'] == planet_name), 'Transits Left in Lifetime'] = \
                            len(p_trans)
                tracker.loc[(tracker['Planet Name'] == planet_name), 'Transits Left in Schedule'] = \
                            len(s_trans)
    ...
    # Remove seconds and below from times
                for j in range(len(start_transits)):
                    start_transits[j] = start_transits[j] - timedelta(seconds=start_transits[j].second,
                                                    microseconds=start_transits[j].microsecond)
                    end_transits[j] = end_transits[j] - timedelta(seconds=end_transits[j].second,
                                                    microseconds=end_transits[j].microsecond)

                early_start  = end_transits - timedelta(hours=20) # Earliest start time to capture transit plus >=4 hours post transit
                late_start = start_transits - timedelta(hours=4)  # Latest start time to capture transit plus >=4 hours pre transit
    ...
    # Check if any transit occurs during observing window
                for j in range(len(early_start)):
                    ...
                    # srt_set = set(start_rng)

                    # overlap_times = obs_set.intersection(srt_set)
                    overlap_times = obs_rng.intersection(start_rng)
                    if len(overlap_times) > 0:

                        # Calc a 'transit factor' 
                        t_left    = tracker.loc[(tracker['Planet Name'] == planet_name), 'Transits Left in Lifetime'].iloc[0]
                        t_need    = tracker.loc[(tracker['Planet Name'] == planet_name), 'Transits Needed'].iloc[0]
...
### Check if there's no transits occuring during the observing window
        if len(temp_df) == 0:
            ...
            if (temp_df['Transit Factor'] < 1.5).any():
                temp_df = temp_df.sort_values(by=['Transit Factor']).reset_index(drop=True)
                print(temp_df['Planet Name'][0], 'Transit Factor Warning')

            # Otherwise sort by Quality Factor and schedule best target
            else:
                temp_df = temp_df.sort_values(by=['Quality Factor']).reset_index(drop=True)
            ...
            tracker.loc[(tracker['Planet Name'] == planet_name), 'Transits Needed'] = \
                (tracker.loc[(tracker['Planet Name'] == planet_name)]['Transits Needed']-1)
            tracker.loc[(tracker['Planet Name'] == planet_name), 'Transits Acquired'] = \
                (tracker.loc[(tracker['Planet Name'] == planet_name)]['Transits Acquired']+1)
            tracker.loc[(tracker['Planet Name'] == planet_name), 'Transit Priority'] = \
                tracker['Transits Left in Lifetime'][i] - tracker['Transits Needed'][i]

            print('Scheduled the ', tracker.loc[(tracker['Planet Name'] == planet_name), 'Transits Acquired'].iloc[0],
                ' transit of ', planet_name)
            start = obs_stop
            stop  = start + obs_window
            continue
