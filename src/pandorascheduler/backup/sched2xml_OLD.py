...
schedule_path = f'{PACKAGEDIR}/data/Pandora_Schedule_0.8_0.0_0.2_2025-11-15.csv'#Pandora_Schedule_2025-08-04_2months.csv'#Pandora_Schedule_2025-08-04.csv'
...
tar_path = f'{PACKAGEDIR}/data/primary-exoplanet-extended_targets.csv'
...
for i in tqdm(range(0, 10)):#, position = 0, leave = True):#len(sch))):#3)):#len(18,19)):#
    ...
    if len(v_change) == 0:
        ...
        if stop_format - start_format <= dt:
            ...
            aa = helper_codes.observation_sequence(visit, "001", target_, priority_, start_format, stop_format, ra_, dec_, targ_info)
            long_sequence = 0
        # If first sequence longer than dt, break it into sections of length dt:
            ...
            for ii, jj in zip(oc_starts_dt, oc_stops_dt):
                ...
                aa = helper_codes.observation_sequence(visit, f'{("0"*(3-len(str(long_sequence))))+str(long_sequence)}', target_, priority_, ii, jj, ra_, dec_, targ_info)
                long_sequence += 1
        ...
        for v in range(len(v_change)-1):
            ...
            if v_t[v+1]: 
                ...
                aa = helper_codes.observation_sequence(visit, f'{("0"*(3-len(str(os_i))))+str(os_i)}', target_, priority_, start_format, stop_format, ra_, dec_, targ_info)
            ...
            # #otherwise, set elements for an occultation target
                ...
                if nn <= 1:
                    ...
                    for ii, jj in zip(oc_starts_dt, oc_stops_dt):
                        target_, ra_, dec_ = info['Target'][oc_tr], info['RA'][oc_tr], info['DEC'][oc_tr]
                        priority_ = f'{oc_flag}'
                        aa = helper_codes.observation_sequence(visit, f'{("0"*(3-len(str(os_i + long_sequence))))+str(os_i + long_sequence)}', \
                            target_, priority_, ii, jj, ra_, dec_, targ_info)
                        long_sequence += 1
                        oc_tr+=1
