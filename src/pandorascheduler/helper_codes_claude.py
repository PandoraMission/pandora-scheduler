...
def visualize_schedule(schedule_data, output_dir, filename):
    """
    Creates a visualization of the observation schedule and saves it as an image file.
    
    :param schedule_data: List of tuples, each containing:
                          (target_name, start_time, stop_time, is_visible)
    ...
    # Convert schedule data into a format suitable for plotting
    targets = []
    starts = []
    durations = []
    colors = []
    ...
    for target, start, stop, is_visible in schedule_data:
        ...
        # Debug print
        print(f"Target: {target}, Start: {start}, Stop: {stop}, Duration: {duration:.2f} hours, Visible: {is_visible}")
    
    # Create the Gantt chart
    bars = ax.barh(targets, durations, left=starts, height=0.5, align='center', color=colors, alpha=0.8)
    
    # Customize the chart
    ax.set_ylim(bottom=-1, top=len(targets))
    ...
    # Debug print
    print(f"Number of bars plotted: {len(bars)}")
